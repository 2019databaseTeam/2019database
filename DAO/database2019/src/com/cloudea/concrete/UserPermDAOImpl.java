...
public class UserPermDAOImpl extends SqlServerConnector implements UserPermDAO {
	...
	private static final String insertSql="insert into User_Perm values(?,?)";
	private static final String deleteSql="delete from User_Perm where u_id = ? and pmt_id = ?";
	private static final String selectSql="select * from User_Perm where u_id=? and pmt_id=?";
	...
	public List<UserPerm> selectAll() throws Exception {
		String sql="select * from User_Perm";
	...
	public List<UserPerm> selectAll(String condition) throws Exception {
		String sql="select * from User_Perm "+condition;
		PreparedStatement statement = connect().prepareStatement(sql);
		List<UserPerm>UserPerms=new ArrayList<UserPerm>();
		ResultSet result=statement.executeQuery();
		while(result.next())
	...
	@Override
	public List<UserPerm> selectAll(int u_id,int pmt_id) throws Exception {
		String sql="select * from User_Perm where u_id="+String.valueOf(u_id)+"and pmt_id="+String.valueOf(pmt_id);
		PreparedStatement statement = connect().prepareStatement(sql);
		List<UserPerm>UserPerms=new ArrayList<UserPerm>();
		ResultSet result=statement.executeQuery();
		while(result.next())
	...
	{
		UserPerm perm = new UserPerm(1,5);
